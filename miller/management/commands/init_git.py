...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            repo.git.config(
                "--global", "--add", "safe.directory", settings.MILLER_CONTENTS_ROOT
            )
        ...
        for prefix in ["story", "document"]:
            prefixed_path = os.path.join(settings.MILLER_CONTENTS_ROOT, prefix)
        ...
        with open(
            os.path.join(settings.MILLER_CONTENTS_ROOT, ".gitignore"), "w"
        ) as file:
            file.write(".DS_Store")
